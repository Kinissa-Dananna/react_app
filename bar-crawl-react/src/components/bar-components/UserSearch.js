...
class UserSearch extends Component {
    ...
    autocompleteUser(event) {
        ...
        axios.get(`http://localhost:8080user-search/${userInput}?auth_token=${this.props.user.token}`)
            .then(response => {
    ...
    onClickUser(user){
      ...
      console.log(userId);
        ...
        axios.post(`http://localhost:8080events/${eventId}/newuser?auth_token=${this.props.user.token}`,
            { eventId, userId })
            .then(response => {
